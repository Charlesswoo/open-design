...
> **[Claude Design][cd]의 오픈소스 대안.** 로컬 우선, 웹 배포 가능, 모든 레이어에서 BYOK — `PATH`에서 자동 감지되는 **15개의 코딩 에이전트 CLI**(Claude Code, Codex, Devin for Terminal, Cursor Agent, Gemini CLI, OpenCode, Qwen, GitHub Copilot CLI, Hermes, Kimi, Pi, Kiro, Kilo, Mistral Vibe, DeepSeek TUI)가 **31개의 조합 가능한 Skill**과 **72개의 브랜드급 디자인 시스템**으로 구동되는 디자인 엔진이 됩니다. CLI가 하나도 없다? OpenAI 호환 BYOK 프록시가 spawn만 빠진 동일한 루프를 돌립니다.
...
| **BYOK 폴백** | `/api/proxy/{anthropic,openai,azure,google}/stream` — `baseUrl` + `apiKey` + `model`을 붙여 넣고 Anthropic / OpenAI / Azure OpenAI / Google Gemini 중 하나를 선택하면 daemon이 SSE를 동일한 채팅 스트림으로 정규화합니다. daemon 경계에서 내부 IP / SSRF를 차단합니다. |
| **내장 디자인 시스템** | **129개** — 2개의 수작업 스타터 + [`awesome-design-md`][acd2]에서 가져온 70개의 제품 시스템(Linear, Stripe, Vercel, Airbnb, Tesla, Notion, Anthropic, Apple, Cursor, Supabase, Figma, Xiaohongshu …) + [`awesome-design-skills`][ads]에서 `design-systems/`에 직접 추가한 57개의 디자인 스킬 |
...
<sub><b>실시간 할 일 진행</b> — 에이전트의 계획이 실시간 카드로 스트리밍됩니다. <code>in_progress</code> → <code>completed</code> 업데이트가 실시간으로 반영됩니다. 작업 중에도 부담 없이 방향을 바꿀 수 있습니다.</sub>
</td>
</tr>
<tr>
<td width="50%">
...
이것이 [`huashu-design`](https://github.com/alchaincyf/huashu-design)에서 추출한 **Junior-Designer 모드**입니다: 미리 질문을 한꺼번에 하고, 일찍 가시적인 것을 보여주며(와이어프레임에 회색 블록이라도), 사용자가 부담 없이 방향을 바꿀 수 있게 해줍니다. 브랜드 에셋 프로토콜(위치 파악 · 다운로드 · `grep` hex · `brand-spec.md` 작성 · 발성)과 결합하면, 출력이 "AI 자유 창작"에서 "그리기 전에 주의를 기울인 디자이너"처럼 느껴지게 되는 가장 큰 이유입니다.
...
| 에이전트 전송 | `child_process.spawn`; 타입 이벤트 파서: `claude-stream-json`(Claude Code), `copilot-stream-json`(Copilot), `json-event-stream` + 각 CLI 파서(Codex / Gemini / OpenCode / Cursor Agent), `acp-json-rpc`(Devin / Hermes / Kimi / Kiro / Kilo / Mistral Vibe via Agent Client Protocol), `pi-rpc`(Pi via stdio JSON-RPC), `plain`(Qwen Code / DeepSeek TUI) |
| BYOK 프록시 | `POST /api/proxy/{anthropic,openai,azure,google}/stream` → 프로바이더별 upstream API, 정규화된 `delta/end/error` SSE; daemon 경계에서 loopback / link-local / RFC1918 호스트 거부 |
...
Open Design은 브라우저 웹앱 또는 Electron 데스크탑 앱으로 실행할 수 있습니다. 두 모드 모두 동일한 로컬 daemon + 웹 아키텍처를 공유합니다.

### 웹 / 로컬호스트 (기본)

```bash
# 포그라운드 모드 — 라이프사이클 명령을 포그라운드에 유지(로그는 파일에 기록)
pnpm tools-dev run web

# 최근 로그 보기:
pnpm tools-dev logs

# 백그라운드 모드 — daemon + web을 백그라운드 프로세스로 실행
pnpm tools-dev start web
```

기본적으로 `tools-dev`는 사용 가능한 임시 포트에 바인딩하고 시작 시 실제 URL을 출력합니다. 중지 상태에서 고정 포트를 사용하려면:

```bash
pnpm tools-dev run web --daemon-port 17456 --web-port 17573
```

daemon/web이 이미 실행 중이라면 `restart`로 기존 세션에서 포트를 전환합니다:
...
```bash
# daemon + web + desktop을 백그라운드에서 시작
pnpm tools-dev
...
데스크탑 앱은 sidecar IPC를 통해 웹 URL을 자동으로 찾습니다 — 포트 추측이 필요 없습니다.

### 기타 유용한 명령어

| 명령어 | 기능 |
|---|---|
| `pnpm tools-dev status` | 실행 중인 sidecar 상태 표시 |
| `pnpm tools-dev logs` | daemon/web/desktop 로그 테일 표시 |
| `pnpm tools-dev stop` | 실행 중인 모든 sidecar 중지 |
| `pnpm tools-dev restart` | 모든 sidecar 중지 후 재시작 |
| `pnpm tools-dev check` | 상태 + 최근 로그 + 일반 진단 |

고정 포트 재시작, 백그라운드 시작, 전체 트러블슈팅은 [`QUICKSTART.md`](QUICKSTART.md)를 참조하세요.

## 코딩 에이전트에서 Open Design 사용하기

Open Design은 stdio MCP 서버를 제공합니다. Claude Code, Codex, Cursor, VS Code, Antigravity, Zed, Windsurf 또는 MCP 호환 클라이언트에 연결하면 다른 저장소의 에이전트가 로컬 Open Design 프로젝트 파일을 직접 읽을 수 있습니다. 내보내기→첨부 반복 작업이 사라집니다. 에이전트가 프로젝트 인수 없이 `search_files`, `get_file`, `get_artifact`를 호출하면 MCP가 현재 Open Design에 열려 있는 프로젝트(와 파일)를 기본값으로 사용합니다. *"내 앱에 이걸 만들어줘"* 나 *"이 스타일을 맞춰줘"* 같은 프롬프트가 그냥 동작합니다.

**MCP를 쓰는 이유?** 디자인을 반복할 때마다 ZIP을 내보내고 다시 첨부하면 흐름이 끊깁니다. MCP 서버는 디자인 소스 — 토큰 CSS, JSX 컴포넌트, 엔트리 HTML — 를 에이전트가 이름으로 조회할 수 있는 구조화된 API로 직접 노출합니다. 에이전트는 항상 마지막 내보내기의 오래된 사본이 아닌 라이브 파일을 봅니다.

Open Design 앱에서 **Settings → MCP server**를 열면 클라이언트별 설치 흐름이 나옵니다. 패널은 `node` 바이너리의 절대 경로와 daemon이 빌드한 `cli.js`를 모든 스니펫에 포함시키므로, `od`가 PATH에 없는 소스 클론에서도 작동합니다. Cursor는 원클릭 딥링크를 제공하며, 나머지는 각 설정 파일 스키마에 맞는 붙여넣기 JSON 스니펫을 받습니다(Claude Code는 `claude mcp add-json` 한 줄 명령이 포함되어 있어 `~/.claude.json`을 직접 편집할 필요가 없습니다). 서버를 등록한 후 클라이언트를 재시작하거나 리로드해야 서버가 나타납니다.

MCP 도구 호출이 성공하려면 daemon이 로컬에서 실행 중이어야 합니다. Open Design보다 에이전트가 먼저 시작된 경우, Open Design이 올라온 후 에이전트를 재시작해야 라이브 daemon에 접근할 수 있습니다. daemon이 오프라인 상태일 때 도구 호출은 충돌 없이 명확한 `"daemon not reachable"` 오류를 반환합니다.

**보안 모델.** MCP 서버는 읽기 전용입니다 — 파일 읽기, 파일 메타데이터, 검색만 노출하며 디스크에 쓰거나 외부 서비스를 호출하는 기능은 없습니다. 코딩 에이전트의 자식 프로세스로 stdio를 통해 실행되므로, 등록한 MCP 클라이언트는 로컬 Open Design 프로젝트에 대한 읽기 권한을 상속합니다. VS Code 확장 설치처럼 취급하세요: 신뢰할 수 있는 클라이언트만 등록하세요. daemon은 기본적으로 `127.0.0.1`에 바인딩하며, LAN 전체 노출은 명시적인 `OD_BIND_HOST` 옵트인이 필요합니다.
...
│   └── kami-deck.html             ← kami 풍 덱 스타터(양피지 / 잉크 블루 세리프)
│
├── scripts/
...
라이브러리는 [`scripts/sync-design-systems.ts`](scripts/sync-design-systems.ts)를 통해 [`VoltAgent/awesome-design-md`][acd2]에서 가져옵니다. 재실행하면 새로 고침됩니다. 57개의 디자인 스킬은 [`bergside/awesome-design-skills`][ads]에서 가져와 `design-systems/`에 직접 추가되었습니다.
...
- **멀티 프로바이더 BYOK 프록시.** `POST /api/proxy/{anthropic,openai,azure,google}/stream`은 `{ baseUrl, apiKey, model, messages }`를 받아 프로바이더별 upstream 요청을 구성하고, SSE 청크를 `delta/end/error`로 정규화하며, SSRF를 막기 위해 loopback / link-local / RFC1918 목적지를 거부합니다. OpenAI 호환은 OpenAI, Azure AI Foundry, DeepSeek, Groq, MiMo, OpenRouter, 자체 호스팅 vLLM을 커버하며, Azure OpenAI는 배포 URL + `api-version`을 추가하고, Google은 Gemini `:streamGenerateContent`를 사용합니다.
...
| 디자인 시스템 | 독점 | `DESIGN.md`(v0.2 로드맵) | **`DESIGN.md` × 129개 시스템 기본 제공** |
...
| 코멘트 모드 수술적 편집 | ❌ | ✅ | 🟡 부분 — 미리보기 요소 주석 + 채팅 첨부 파일 출시; 정밀 패치 신뢰도는 개선 중 |
| AI 제안 트윅 패널 | ❌ | ✅ | 🚧 로드맵 — 채팅 통합 패널 UX 미구현 |
...
| **멀티 프로바이더 BYOK** | n/a | SSE 정규화 | `POST /api/proxy/{provider}/stream` → Anthropic / OpenAI 호환 / Azure OpenAI / Gemini; loopback / link-local / RFC1918에 대한 SSRF 차단 |
...
| [`VoltAgent/awesome-claude-design`][acd] / [`awesome-design-md`][acd2] | 9섹션 `DESIGN.md` 스키마의 출처이자 [`scripts/sync-design-systems.ts`](scripts/sync-design-systems.ts)를 통해 가져온 70개 제품 시스템. |
| [`bergside/awesome-design-skills`][ads] | `design-systems/`에 정규화된 `DESIGN.md` 파일로 직접 추가된 57개 디자인 스킬의 출처. |
...
- [x] 멀티 프로바이더 BYOK 프록시(`/api/proxy/{anthropic,openai,azure,google}/stream`) + SSRF 차단
...
- [ ] 코멘트 모드 수술적 편집 — 부분 출시: 미리보기 요소 주석 + 채팅 첨부 파일; 정밀 패치 신뢰도는 개선 중
- [ ] AI 제안 트윅 패널 UX — 미구현
...
이것이 30분을 절약해줬다면 — ★를 주세요. 별 하나로 당장 수익이 되진 않지만, 다음 디자이너, 에이전트, 기여자에게 이 실험이 그들의 관심을 받을 가치가 있다는 것을 알려줍니다. 한 번의 클릭, 3초, 진짜 신호: [github.com/nexu-io/open-design](https://github.com/nexu-io/open-design).
